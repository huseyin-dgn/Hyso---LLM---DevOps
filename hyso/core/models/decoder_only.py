...
def key_padding_to_bool(attn_mask_bt: torch.Tensor) -> torch.Tensor:
    """
    [B,T] → bool [B,T] (True = PAD / MASKLANACAK)
    - int/tensor: 1=token, 0=pad  → 0 -> True (pad)
    - bool:      True=pad, False=token (buna göre vermen lazım)
    """
    if attn_mask_bt.dtype == torch.bool:
        # Burada da True=PAD varsayıyoruz.
        return attn_mask_bt
...
class DecoderOnlyLayer(nn.Module):
    def __init__(self,
        ...
        self.alibi_slopes = alibi_slopes  # [h] veya None
    ...
    def _alibi_bias(self,
                    Tq: int,
                    Tk: int,
                    device,
                    dtype,
                    pos_offset: int) -> Optional[torch.Tensor]:
        if not self.use_alibi or self.alibi_slopes is None:
            return None
        i = torch.arange(Tq, device=device, dtype=dtype).unsqueeze(-1) + pos_offset  # [Tq,1]
        j = torch.arange(Tk, device=device, dtype=dtype).unsqueeze(0)               # [1,Tk]
        dist = i - j  # [Tq,Tk]
    ...
    def forward(self,
                x: torch.Tensor,
                *,
                pad_mask: Optional[torch.Tensor] = None,   # [B,T] 1=token,0=pad veya bool True=PAD
                rope: Optional[Tuple[torch.Tensor, torch.Tensor]] = None,
                pos_offset: int = 0,
        ...
        if self.use_alibi:
            ...
            if self_kv_cache is None:
                ...
                Tk = self_kv_cache["k"].size(2) + x.size(1)
            alibi = self._alibi_bias(Tq, Tk, x.device, x.dtype, pos_offset)
        ...
        return x, self_kv_cache


# ---------------------------
# 8) HysoDecoderOnly (causal LLM)
# ---------------------------
class HysoDecoderOnly(nn.Module):
    def __init__(self,
                 vocab_size: int,
                 *,
                 d_model: int = 512,
        ...
        self.use_alibi = bool(use_alibi)
        self.grad_ckpt_dec = bool(grad_ckpt_dec)
        ...
        # Kritik 1: Pozisyon bilgisi yoksa izin verme
        if self.pos_embedding_type == "none" and not self.use_rope:
            raise ValueError(
                "pos_embedding_type='none' ve use_rope=False → modelde hiç pozisyon bilgisi yok. "
                "Bilinçli deney yapmıyorsan bu kombinasyona izin vermiyorum."
            )

        # Kritik 2: RoPE + ALiBi aynı anda karışık → şimdilik yasak
        if self.use_rope and self.use_alibi:
            raise ValueError(
                "use_rope=True ve use_alibi=True aynı anda desteklenmiyor. "
                "Şimdilik ya RoPE ya ALiBi seç."
            )
        ...
        if self.use_alibi:
            slopes = self._alibi_slopes(self.n_heads)
        ...
        self.decoder_layers = nn.ModuleList([
            DecoderOnlyLayer(
                d_model, n_heads, kv_heads=self.kv_heads,
                attn_dropout=attn_dropout, proj_dropout=proj_dropout,
    ...
    def _alibi_slopes(n_heads: int) -> torch.Tensor:
        ...
        m = power_of_2(n_heads)
        m_list = torch.pow(2, -torch.arange(1, m + 1)).tolist()
        slopes = torch.tensor(m_list[:n_heads], dtype=torch.float32)
        return slopes
    ...
    def _maybe_ckpt(self,
        ...
        if not self.grad_ckpt_dec or not self.training:
            return layer(x, pad_mask=pad_mask, rope=rope,
                         pos_offset=pos_offset, self_kv_cache=self_kv_cache)

        def fn(inp):
            y, _ = layer(inp, pad_mask=pad_mask, rope=rope,
                         pos_offset=pos_offset, self_kv_cache=self_kv_cache)
            return y

        y = ckpt(fn, x)
        return y, self_kv_cache

    # --- decode / forward (teacher forcing) ---
    def decode(self,
               input_ids: torch.Tensor,
               attention_mask: Optional[torch.Tensor] = None,
               pos_offset: int = 0) -> torch.Tensor:
        # Kritik: teacher forcing için pos_offset daima 0 olmalı
        if pos_offset != 0:
            raise ValueError(
                "decode şu an sadece pos_offset=0 ile kullanılabilir. "
                "Streaming/step-by-step için generate_step kullan."
            )
        ...
        for layer in self.decoder_layers:
            x, _ = self._maybe_ckpt(layer, x, rope=rope,
                                    pad_mask=pad_mask,
                                    pos_offset=0,
                                    self_kv_cache=None)
    ...
    def forward(self,
        ...
        h = self.decode(input_ids, attention_mask=attention_mask, pos_offset=0)
    ...
    # --- incremental decode + generate ---
    @torch.no_grad()
    def generate_step(self,
                      prev_token: torch.Tensor,      # [B]
                      *,
                      layer_caches: Optional[List[Optional[Dict[str, torch.Tensor]]]] = None,
                      step_idx: int = 0) -> Tuple[torch.Tensor, List[Optional[Dict[str, torch.Tensor]]]]:
        ...
        y = self.token_embed(prev_token).unsqueeze(1) * (self.d_model ** 0.5)

        if self.pos_embedding_type == "learned":
            if step_idx >= self.max_position_embeddings:
                raise ValueError(f"step_idx {step_idx} >= max_position_embeddings {self.max_position_embeddings}")
            pos_vec = self.pos_embed.weight[step_idx]
            y = y + pos_vec.view(1, 1, -1)
        elif self.pos_embedding_type == "sinusoidal":
            if step_idx >= self.max_position_embeddings:
                raise ValueError(f"step_idx {step_idx} >= max_position_embeddings {self.max_position_embeddings}")
            pos_vec = self.pos_sinus[step_idx]
            y = y + pos_vec.view(1, 1, -1)
        ...
        rope = (self.rope_cos, self.rope_sin) if self.use_rope else None
        if rope is not None:
            self.ensure_rope_len(step_idx + 1)

        x = y
        new_caches: List[Optional[Dict[str, torch.Tensor]]] = []
        for i, layer in enumerate(self.decoder_layers):
            cache_in = layer_caches[i]
            x, cache_out = layer(
                x,
                pad_mask=None,
                rope=rope,
                pos_offset=step_idx,
                self_kv_cache=cache_in,
            )
    ...
    def generate_greedy(self,
        ...
        # Kritik: generate için padding istemiyoruz
        if (input_ids == self.pad_id).any():
            raise ValueError(
                "generate_greedy içinde pad_id tespit edildi. "
                "Generate için padding’siz / trimlenmiş prompt kullan."
            )
        ...
        prev = input_ids[:, 0]
        if T0 > 1:
            for step in range(T0 - 1):
                _, caches = self.generate_step(prev, layer_caches=caches, step_idx=step)
                prev = input_ids[:, step + 1]

        generated: List[torch.Tensor] = []
        step = T0 - 1 if T0 > 0 else 0
        for _ in range(max_new_tokens):
            logits, caches = self.generate_step(prev, layer_caches=caches, step_idx=step)
        ...
        if generated:
            new_seq = torch.stack(generated, dim=1)
            return torch.cat([input_ids, new_seq], dim=1)
        else:
            return input_ids
    ...
    def generate_sampling(self,
        ...
        if (input_ids == self.pad_id).any():
            raise ValueError(
                "generate_sampling içinde pad_id tespit edildi. "
                "Generate için padding’siz / trimlenmiş prompt kullan."
            )
        ...
        for _ in range(max_new_tokens):
            ...
            if top_k > 0:
                k_val = min(top_k, probs.size(-1))
                topk_vals, topk_idx = torch.topk(probs, k_val, dim=-1)
...
TransformerDec = HysoDecoderOnly